...
class OpenOceanStandalone:
    ...
    async def get_quote(
        ...
        Args:
            from_token_address: The token address to swap from 
    ...
    async def build_swap_transaction(
        ...
        if 'maxFeePerGas' in swap_data and 'maxPriorityFeePerGas' in swap_data:
            tx_params['maxFeePerGas'] = Web3.to_wei(
                float(swap_data['maxFeePerGas']), 
                'gwei'
            )
            tx_params['maxPriorityFeePerGas'] = Web3.to_wei(
                float(swap_data['maxPriorityFeePerGas']), 
                'gwei'
            )
    ...
    async def execute_swap(
        ...
        tx_params = await self.build_swap_transaction(
            from_token_address, 
            to_token_address, 
            amount, 
            slippage
...
async def example_swap():
    """Example usage of the OpenOceanStandalone class."""
    # Configuration
    ...
    try:
        ...
        print(
            f"Quote: {Web3.from_wei(int(quote['outAmount']), 6)} USDC "
            f"for {Web3.from_wei(amount, 'ether')} ETH"
        )
